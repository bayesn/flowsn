...
def sample_model(n_sne, m0_prior="halfnormal", m_cut=24.0, sigma_cut=0.25, a=-0.1, b=-1.0):
    with numpyro.plate("plate_i", n_sne):
        ...
        sigma_res, sigma_c, sigma_x = numpyro.sample("sigma_res", dist.Uniform(1e-4, 0.2)), numpyro.sample("sigma_c", dist.Uniform(1e-4, 0.1)), numpyro.sample("sigma_x", dist.Uniform(1e-4, 1.5))
        ...
        m_s = numpyro.sample("m_s", dist.Normal(m0 + alpha * x_s + beta * c_s, sigma_res))
...
def build_training_rows(pred):
    ...
    params = jnp.column_stack((pred["m0"][0][sel], pred["c0"][0][sel], pred["x0"][0][sel], jnp.log(pred["sigma_res"][0][sel]), jnp.log(pred["sigma_c"][0][sel]), jnp.log(pred["sigma_x"][0][sel]), pred["alpha"][0][sel], pred["beta"][0][sel], pred["alpha_c"][0][sel], pred["log_mag_err_s"][0][sel], pred["log_c_err_s"][0][sel], pred["log_x_err_s"][0][sel], pred["cov_m_c"][0][sel], pred["cov_m_x"][0][sel], pred["cov_c_x"][0][sel]))
